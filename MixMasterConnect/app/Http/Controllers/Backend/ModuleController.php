...
class ModuleController extends Controller
{
    ...
    public function showAll(Request $request)
    {
        ...
        $totalData = Module::whereNull('deleted_at')->count();
        ...
        if( empty($request->input('search.value')) ) {
            $items = Module::whereNull('deleted_at')
                    ->offset($start)
                    ->limit($limit)
                    ->orderBy($order,$dir)
                    ->get();
            ...
            $totalFiltered = Module::whereNull('deleted_at')
                    ->where('id','like',"%{$search}%")
                    ->orWhere('module_code', 'like',"%{$search}%")
                    ->orWhere('module_name', 'like',"%{$search}%")
                    ->orWhere('is_active', 'like',"%{$search}%")
        ...
        if( !empty($items) ) {
            ...
            foreach ($items as $item)
            {
                if($item->deleted_at == null && $item->deleted_at == ''){
                    $values['id'] = $item->id;
                    $values['module_code'] = $item->module_code;
                    $values['module_name'] = $item->module_name;
                    ...
                    if(isset($permission)){
                        ...
                        $values['action'] = $values['action'].'<button title="Edit" class="edit btn btn-icon btn-success btn-sm mr-2" data-id='.$item->id.' style="margin-right:3px"><i class="bi bi-pencil-fill"></i></button>';
                        }
                    }
                    $data[] = $values;
                }
